/**
 * Взято из прекрасной статьи про Emojis в javascript
...
const supplemental = Array.from<string>([]).concat(
    rsDingbat,
    rsRegional,
    rsSurrPair,
...
export const templRegExpEmoji = '(?:' + supplemental + ')';


const urlRegex = '(^|\s)https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)'
const hashtagsRegex = '(^|\s)(#[a-z\d-]+)'
const MenschenRegex = '(^|\s)(@[a-z\d-]+)'
const mailRegex = `\S+@\S+\.\S+`

let tempHighlightArr = Array.from<string>([]).concat(
    urlRegex,
    hashtagsRegex,
    MenschenRegex,
    mailRegex,
...
export const tempHighlight = mailRegex